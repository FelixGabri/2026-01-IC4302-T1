-- Se seleciona la base de datos IC4302
USE ic4302;

-- Se eliminan las tablas en orden de dependencias para evitar problemas al hacerlo
...
-- Almacena los paises disponibles del sistema

CREATE TABLE Countries (
   ID int AUTO_INCREMENT PRIMARY KEY,
   name varchar(40) NOT NULL,
   branch_amount int, -- Sucursales
   phone_extension varchar(40) NOT NULL -- Codigo telefonico
);

-- Almacena los paises del sistema
...
CREATE TABLE Users (
   ...
   -- Contraint asegurara que tenga un pais existente
   CONSTRAINT fk_UserCountry FOREIGN KEY (country_ID) REFERENCES Countries (ID) -- FK al ID del pais al que pertenece el usuario
);

-- Almacenara los telefonos registrados

CREATE TABLE Phones (
   ID int AUTO_INCREMENT PRIMARY KEY,
   number varchar(40),
   country_ID int NOT NULL,
   CONSTRAINT fk_PhoneCountry FOREIGN KEY (country_ID) REFERENCES Countries (ID),
   UNIQUE (number, country_ID) -- Evita duplicados para el mismo pais
);

-- Tabla de relacion telefono dueño

CREATE TABLE Phone_Ownership (
   owner_ID int NOT NULL,
   number_ID int NOT NULL,
   CONSTRAINT fk_PhoneOwner FOREIGN KEY (owner_ID) REFERENCES Users (ID), -- Id del usuario
   CONSTRAINT fk_PhoneNumber FOREIGN KEY (number_ID) REFERENCES Phones (ID), -- Id del telefono
...
-- Almacena los modelos de vehiculos

CREATE TABLE Models (
   ID int AUTO_INCREMENT PRIMARY KEY,
   maker_ID int NOT NULL,
   name varchar(40) NOT NULL UNIQUE,
   CONSTRAINT fk_ModelMaker FOREIGN KEY (maker_ID) REFERENCES Makers (ID)
);

-- Almacena los vehiculos registrados
...
CREATE TABLE Cars (
   ...
   year int,
   country_ID int NOT NULL,
   CONSTRAINT fk_CarModel FOREIGN KEY (model_ID) REFERENCES Models (ID), -- Id del modelo
   CONSTRAINT fk_CarCountry FOREIGN KEY (country_ID) REFERENCES Countries (ID) -- Id del pais
);

-- Tabla para relacionar usuario con el carro

CREATE TABLE Car_Ownership (
   owner_ID int NOT NULL,
   car_plate varchar(40) NOT NULL,
   CONSTRAINT fk_CarOwner FOREIGN KEY (owner_ID) REFERENCES Users (ID), -- Id al usuario
   CONSTRAINT fk_CarPlate FOREIGN KEY (car_plate) REFERENCES Cars (plate), -- Id de la placa del carro
